...
def topic_parser(href):
    ...
    tags = str(topic_page("div", {"class", "topic-tags"})[0].text)
    # ПАРСИМ
    ...
    for item in content_header:
        ...
        # ТЕГИ ТЕКСТ
        # topic_tag = topic_tag_html.find("a", {"class", "tag"}).text
        # ID 1-го изображения
        ...
        try:
            video_1_data_scr = \
                topic_contents.find("div", {"class": "first-video"}).find("div", {"class": "module-video"}).find("div",
                                                                                                                 {
    ...
    return img_1_href, channel, title, text_1, img_2_href, img_3_href, video_1_title, video_1_data_scr, tags
