...
class Topics:
    ...
    re_front_img_src = re.compile(
        r'src=\"https://dw8stlw9qt0iz\.cloudfront\.net/(.*?)/fit-in/380x0/filters:format\(jpeg\):quality\(70\)/https:(.*?)\"')
    re_front_img_alt = re.compile(r'alt=\"(.*?)\"')
    re_first_post_img = re.compile(
        r'<img class="flipboard-image" src="https://dw8stlw9qt0iz.cloudfront.net/(.*?)/750x450/filters:format\(jpeg\):quality\(75\)/(.*?)\"')
    ...
    def get_full_topic_html():
        ...
        while count <= max_index:
            ...
            open('/home/ubuntu/workspace/curiosity-to-vk/topics/html/' + str(count) + '.html', 'w').write(
                Topics.full_topic_html[count])
            count = count + 1
    ...
    def download_front_img():
        ...
        g_front_img.setup(headers={
            'chrome-proxy': 's=Ci4KEwjMhrTr79_TAhXQIBkKHQH0A0USDAib-sXIBRCojrrZAxoJCgdkZWZhdWx0EkgwRgIhAIdAgOu3URu5cs-VjgT5MlOXD8ckBB3udUqTMeXYIGc0AiEAmdK5TxX33uuUGLrtjrZGzKNXOnV4NIJg6YKU50BVMsA=, c=win, b=3029, p=96'})
        count = 0
        max_index = 9
        while count <= max_index:
            Topics.front_img_href.append("http:" + Topics.front_img_srcs[count][1])
            resp = g_front_img.go(Topics.front_img_href[count])
            open('/home/ubuntu/workspace/curiosity-to-vk/topics/front-img/' + str(count) + '.png', 'wb').write(
                resp.body)
            Topics.front_img_src.append(
                '/home/ubuntu/workspace/curiosity-to-vk/topics/front-img/' + str(count) + '.png')
            count = count + 1
    ...
    def get_first_img_src():
        ...
        g_first_post_img.setup(headers={
            'chrome-proxy': 's=Ci4KEwjMhrTr79_TAhXQIBkKHQH0A0USDAib-sXIBRCojrrZAxoJCgdkZWZhdWx0EkgwRgIhAIdAgOu3URu5cs-VjgT5MlOXD8ckBB3udUqTMeXYIGc0AiEAmdK5TxX33uuUGLrtjrZGzKNXOnV4NIJg6YKU50BVMsA=, c=win, b=3029, p=96'})
        count = 0
        max_index = 9
    ...
    def download_first_img():
        ...
        while count <= max_index:
            ...
            open('/home/ubuntu/workspace/curiosity-to-vk/topics/first-img/' + str(count) + '.jpg', 'wb').write(
                resp.body)
            count = count + 1
    ...
    def transya():
        ...
        for count in range(9):
            ...
            Topics.front_img_alt_ru = json.loads(g_translate_img_alt.go(
            ...
            resp_tr_title = json.loads(g_translate_img_title.go(
                'https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20170514T220842Z.5b2c14ecd7990670.3ccb355751262f1359f3c3ff0b9b7d5447ce39a1',
                post={"text": Topics.curi_tr_get_topic_title()[count], 'lang': 'en-ru',
                      'format': 'plain'}).unicode_body(ignore_errors=True, fix_special_entities=True))
    ...
    def topic_obj_gen():
        ...
        for count in max_index:
            yield Topics(curi_tr_html_get_topic_href()[count], curi_tr_get_topic_title()[count],
                         download_front_img()[count], curi_tr_get_topic_front_img_alt()[count], )
